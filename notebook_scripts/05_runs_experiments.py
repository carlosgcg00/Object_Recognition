# notebook_scripts/05_runs_experiments.py
import sys
...
for id_model, (SELECTED_MODEL, model_config) in enumerate(list(selected_models_efficientdet.items())):
    if SELECTED_MODEL == 'tf_efficientdet_d2':
        ...
        for idx_experiment, experiment in enumerate(list_experiments[-1:]):
            ...
            for fold_index, fold_config_yaml_file in enumerate(fold_config_yaml_files):
                if fold_index >= 3:
                    ...
                    if fold_index == 0 and SELECTED_MODEL == 'tf_efficientdet_d2' and idx_experiment == 0:
                        fold_name = fold_config_yaml_file.stem
